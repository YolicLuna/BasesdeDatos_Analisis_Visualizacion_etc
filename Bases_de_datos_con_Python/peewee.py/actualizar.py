...
if __name__ == "__main__":
    ...
    # Se actualizan los datos de un usuario
    user = User.select().where(User.id == 1).get()

    # Se modifican los valores de los campos
    user.username = "user1_updated"
    user.email = "user1_updated@ejemplo.com"

    # Se guarda el objeto en la base de datos
    user.save()

    # Se actualizan los datos de un usuario con el metodo update
    query = User.update(username = "user2_updated", 
    email = "user2_updated@ejemplo.com").where(User.id == 2)
