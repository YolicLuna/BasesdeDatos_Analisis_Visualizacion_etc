...
# Se crea la clase User que hereda de peewee.Model
...
if __name__ == "__main__":
    ...
    # Se crea un objeto de la clase User con el metodo create, se guarda el objeto en la base de datos
    ...
    """Se crea una consulta condicionada con el metodo User.select() y el metodo where(), se recorre el resultado y se 
    imprimen sus atibutos o campos. Esto es equivalente a SELECT username, email, active FROM users WHERE active = True"""
    users = User.select(User.username, User.email, User.active).where(User.active == True)
    for user in users:
        print(user)

    """Se crea una consulta condicionada con el metodo User.select() y el metodo where(), se recorre el resultado y se 
    imprimen sus atibutos o campos. Esto es equivalente a SELECT username, email, active FROM users WHERE active = True AND id > 6"""
    users = User.select(User.username, User.email, User.active).where((User.active == True) & (User.id > 6))
    for user in users:
        print(user)
    
    """Se crea una consulta condicionada con el metodo User.select(), el metodo where(), el metodo order_by() y el metodo 
    limit(), se recorre el resultado y se imprimen sus atibutos o campos. Esto es equivalente a SELECT * FROM users WHERE active = True ORDER BY username DESC LIMIT 2"""
